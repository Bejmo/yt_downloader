...
class Worker(QThread):
    ...
    stop_signal = False # Indica si se tiene que parar la ejecución
    
    def __init__(self, stop_flag, func): # func = función que se quiere ejecutar
        super().__init__()
...
class MyApp(QWidget):
    ...
    stop_flag = False # Indica si se para el proceso que se está realizando
    ...
    def __init__(self):
        ...
        self.ui.stop_descarga_button.pressed.connect(self.stop_task)
    ...
    def descargar(self):
        ...
        if (self.estado == 1):
            yt = YouTube(url)
            self.thread = Worker(self.stop_flag, descargar_audio_youtube(yt, directorio, 0, self))
        elif (self.estado == 2):
            p = Playlist(url)
            self.thread = Worker(self.stop_flag, descargar_playlist(p, directorio, self))
        elif (self.estado == 3):
            p = Playlist(url)
            self.thread = Worker(self.stop_flag, actualizar_playlist(p, directorio, self))
    ...
    def stop_task(self):
        if self.thread.isRunning():
            self.thread.stop()
            self.ui.terminal.append('Operación detenida.\n')
